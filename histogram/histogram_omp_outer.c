...
static void omp_compute_histogram_outer(const ELEMENT_TYPE *array, int *histogram, struct s_settings *p_settings)
{
        memset(histogram, 0, p_settings->nb_bins * sizeof(*histogram));
...
{
        omp_compute_histogram_outer(array, run_histogram, p_settings);
