...
static void usage(void)
{
        ...
        fprintf(stderr, "    --upper-bound  UPPER_BOUND\n");
...
static void init_array_random(ELEMENT_TYPE *array, struct s_settings *p_settings)
{
        ...
        const ELEMENT_TYPE value_range = p_settings->upper_bound - p_settings->lower_bound;

        int i;
        for (i = 0; i < p_settings->array_len; i++)
        {
                ELEMENT_TYPE value = value_range * ((ELEMENT_TYPE)rand()) / (1.0 + (ELEMENT_TYPE)(RAND_MAX)) + offset;
                array[i] = value;
        }
}
...
static void print_histogram(const int *histogram, struct s_settings *p_settings)
{
        ...
        {
                ELEMENT_TYPE lower = offset + i * value_range / p_settings->nb_bins;
                ELEMENT_TYPE upper = offset + (i + 1) * value_range / p_settings->nb_bins;
...
static void write_bins_to_file(FILE *file, struct s_settings *p_settings)
{
        ...
        for (i = 0; i < p_settings->nb_bins; i++)
        {
                ELEMENT_TYPE bound = offset + (i + 1) * value_range / p_settings->nb_bins;
                ret = fprintf(file, "%lf\n", bound);
                IO_CHECK("fprintf", ret);
        }
}
...
static void naive_compute_histogram(const ELEMENT_TYPE *array, int *histogram, struct s_settings *p_settings)
{
...

        {
                ...
                {
                        bounds[j + 1] = offset + (j + 1) * value_range / p_settings->nb_bins;
                }
        }
...
__global__ void histogram_kernel(const ELEMENT_TYPE *array, int *histogram, const ELEMENT_TYPE *bounds, int size, const int nb_bins, ELEMENT_TYPE offset, ELEMENT_TYPE bin_width)
{
    extern __shared__ int tmp_histogram[];

    // Initialize shared memory
    for (int idx = threadIdx.x; idx < nb_bins; idx += blockDim.x)
    {
        tmp_histogram[idx] = 0;
    }
    ...
    if (id < size)
    {
        ELEMENT_TYPE value = array[id];
        int bin_index = (int)((value - offset) * bin_width) - 1;

        for (int i = bin_index; i < nb_bins; i++)
        {
            if (value >= bounds[i] && value < bounds[i + 1])
            {
                atomicAdd(tmp_histogram + i, 1);
                break;
            }
        }
    }
    ...
    // Write back to global memory using atomicAdd
    for (int idx = threadIdx.x; idx < nb_bins; idx += blockDim.x)
    {
        atomicAdd(histogram + idx, tmp_histogram[idx]);
    }
}

// Function to compute histogram on GPU
static void cuda_compute_histogram(const ELEMENT_TYPE *input_array, int *output_histogram, struct s_settings *settings)
{
    // Allocate device memory
    ELEMENT_TYPE *device_array;
    int *device_histogram;
    ELEMENT_TYPE *device_bounds;

    cudaMalloc((void **)&device_array, settings->array_len * sizeof(ELEMENT_TYPE));
    cudaMalloc((void **)&device_histogram, settings->nb_bins * sizeof(int));
    cudaMalloc((void **)&device_bounds, (settings->nb_bins + 1) * sizeof(ELEMENT_TYPE));

    // Copy input data to device
    cudaMemcpy(device_array, input_array, settings->array_len * sizeof(ELEMENT_TYPE), cudaMemcpyHostToDevice);

    // Calculate bounds on host
    ELEMENT_TYPE *bounds = (ELEMENT_TYPE *)malloc((settings->nb_bins) * sizeof(*bounds));
    if (bounds == NULL)
    {
        PRINT_ERROR("Memory allocation failed");
    }

    const ELEMENT_TYPE offset = settings->lower_bound;
    const ELEMENT_TYPE value_range = settings->upper_bound - settings->lower_bound;

    bounds[0] = offset;

    for (int i = 0; i < settings->nb_bins; i++)
    {
        bounds[i + 1] = offset + (i + 1) * value_range / settings->nb_bins;
    }

    // Copy bounds to device
    cudaMemcpy(device_bounds, bounds, (settings->nb_bins + 1) * sizeof(ELEMENT_TYPE), cudaMemcpyHostToDevice);

    const ELEMENT_TYPE bin_width = settings->nb_bins / value_range;

    // Launch kernel with dynamic shared memory allocation
    int nb_blocks = (settings->array_len + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
    histogram_kernel<<<nb_blocks, THREADS_PER_BLOCK, settings->nb_bins * sizeof(int)>>>(device_array, device_histogram, device_bounds, settings->array_len, settings->nb_bins, offset, bin_width);

    // Copy result back to host
    cudaMemcpy(output_histogram, device_histogram, settings->nb_bins * sizeof(int), cudaMemcpyDeviceToHost);
