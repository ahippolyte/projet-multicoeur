...
static void omp_compute_histogram_inner(const ELEMENT_TYPE *array, int *histogram, struct s_settings *p_settings)
{
        memset(histogram, 0, p_settings->nb_bins * sizeof(*histogram));
...
{
        omp_compute_histogram_inner(array, run_histogram, p_settings);
