...
#define DEFAULT_STARPU_PARTS 10
...
static void usage(void)
{
        ...
        fprintf(stderr, "    --nb-partitions MESH_HEIGHT\n");
...
static void init_settings(struct s_settings **pp_settings)
{
        ...
        p_settings->nb_partitions = DEFAULT_STARPU_PARTS;
...
static void parse_cmd_line(int argc, char *argv[], struct s_settings *p_settings)
{
        ...
        while (i < argc)
        {
                ...
                else if (strcmp(argv[i], "--nb-partitions") == 0)
                {
                        i++;
                        if (i >= argc)
                        {
                        ...
                        if (value < 0)
                        {
                                fprintf(stderr, "invalid DEFAULT_STARPU_PARTS argument\n");
                                exit(EXIT_FAILURE);
                        }
                        p_settings->nb_partitions = value;
                }
...
{
        printf("nb_partitions,mesh_width,mesh_height,nb_iterations,nb_repeat");
}

static void print_settings_csv(struct s_settings *p_settings)
{
        printf("%d,%d,%d,%d,%d", p_settings->nb_partitions, p_settings->mesh_width, p_settings->mesh_height, p_settings->nb_iterations, p_settings->nb_repeat);
}
...
static void starpu_stencil_func(ELEMENT_TYPE *p_mesh_read, ELEMENT_TYPE *p_mesh_write, struct s_settings *p_settings)
{
        int p_mesh_write_width = p_settings->mesh_width - 2;
        int p_mesh_write_height = p_settings->mesh_height - 2;

        starpu_data_handle_t handle_read, handle_write;
        starpu_matrix_data_register(
                &handle_read,
                STARPU_MAIN_RAM,
                (uintptr_t)p_mesh_read,
        ...
        starpu_matrix_data_register(
                &handle_write,
                STARPU_MAIN_RAM,
                (uintptr_t)(p_mesh_write + p_settings->mesh_width + 1),
                p_settings->mesh_width,
                p_mesh_write_width,
                p_mesh_write_height,
                sizeof(ELEMENT_TYPE)
        ...
        struct starpu_data_filter filter_read =
        {
            .filter_func = starpu_matrix_filter_vertical_block_shadow,
            .nchildren = p_settings->nb_partitions,
            // shadow thickness
            .filter_arg_ptr = (void *)(uintptr_t)1
        };

        starpu_data_partition(handle_read, &filter_read);

        struct starpu_data_filter filter_write =
        {
            .filter_func = starpu_matrix_filter_vertical_block,
            .nchildren = p_settings->nb_partitions,
        };

        starpu_data_partition(handle_write, &filter_write);

        int i;
        for (i = 0; i < p_settings->nb_partitions; i++)
        {
                starpu_data_handle_t sub_handle_read = starpu_data_get_sub_data(handle_read, 1, i);
                starpu_data_handle_t sub_handle_write = starpu_data_get_sub_data(handle_write, 1, i);

                starpu_task_insert(&cl, STARPU_R, sub_handle_read, STARPU_W, sub_handle_write, 0);
        }

        starpu_task_wait_for_all();
        starpu_data_unpartition(handle_read, STARPU_MAIN_RAM);
        starpu_data_unpartition(handle_write, STARPU_MAIN_RAM);
        starpu_data_unregister(handle_read);
        starpu_data_unregister(handle_write);
}
...
static void run(ELEMENT_TYPE *p_mesh, struct s_settings *p_settings)
{
        ...
        // Allocate the writing buffer
        ELEMENT_TYPE *p_mesh_write = malloc(p_settings->mesh_width * p_settings->mesh_height * sizeof(*p_mesh_write));
        apply_boundary_conditions(p_mesh_write, p_settings);

        int i;
        for (i = 0; i < p_settings->nb_iterations; i++)
        {
                starpu_stencil_func(p_mesh, p_mesh_write, p_settings);

                // swap buffers
                ELEMENT_TYPE *tmp = p_mesh_write;
                p_mesh_write = p_mesh;
                p_mesh = tmp;
        ...
        // copy the content of p_mesh_write in p_mesh when needed
        if (i % 2)
        {
                memcpy(p_mesh_write, p_mesh, p_settings->mesh_width * p_settings->mesh_height * sizeof(*p_mesh));
                free(p_mesh);
        }
        else
        {
